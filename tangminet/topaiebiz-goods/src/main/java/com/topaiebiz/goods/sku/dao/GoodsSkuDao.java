...
public interface GoodsSkuDao extends BaseDao<GoodsSkuEntity> {
	...
	* @param:
	**/
	Integer reduceStock(@Param("skuId") Long skuId,@Param("number") Integer number);

	Integer backStock(@Param("skuId") Long skuId,@Param("number") Integer number);
	/**
	 ...
	 * @return
	 */
    List<GoodsSkuDTO> selectGoodsSkuDTO(Long itemId);

	List<GoodsSkuEntity> selectSkuList(@Param("skuIds") List<Long> skuIds);
}
